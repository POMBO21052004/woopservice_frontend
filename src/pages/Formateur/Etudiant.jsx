...
export default function ViewEtudiantsByFormateurs() {
  // États
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const res = await api.get("/formateur/view/etudiant", { params });
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({});
    try {
      await api.post("/formateur/store/etudiant", form);
      fetchUsers();
  ...
  const handleDeleteConfirmed = async () => {
    try {
      ...
      if (userToDelete) {
        await api.delete(`/formateur/destroy/etudiant/${userToDelete}`);
      } else {
        await api.post(`/formateur/destroy-group/etudiant`, { ids: idsToDelete });
      }
  ...
  const handleToggleRole = async (userId) => {
    try {
      const res = await api.patch(`/formateur/toggle-role/etudiant/${userId}`);
      fetchUsers();
  ...
  const handleRoleGroupChange = async (e) => {
    e.preventDefault();
    try {
      const res = await api.post(`/formateur/toggle-role-group/etudiant`, {
        ids: checkedUsers,
  ...
  const handleAssignClassroom = async (userId, matriculeClassroom) => {
    try {
      const res = await api.patch(`/formateur/assign-classroom/etudiant/${userId}`, {
  ...
  const handleClassroomGroupAssign = async (e) => {
    e.preventDefault();
    try {
      const res = await api.post(`/formateur/assign-classroom-group/etudiant`, {
        ids: checkedUsers,
  ...
  const handleDownloadPdf = async (e) => {
    ...
    try {
      ...
      const res = await api.get(`/formateur/download/liste/etudiant/pdf?${params.toString()}`, {
        responseType: "blob",
      });
  ...
  const handleDownloadExcel = async (e) => {
    ...
    try {
      ...
      const res = await api.get(`/formateur/download/liste/etudiant/excel?${params.toString()}`, {
        responseType: "blob",
      });
                        ...
                        <td className="text-center">
                          <div className="d-flex gap-1 justify-content-center">
                            <Button
                              ...
                              as={Link}
                              to={`/formateur/show/etudiant/${user.id}`}
                            >
                            ...
                            <Button
                              ...
                              title="Modifier"
                              as={Link}
                              to={`/formateur/edit/etudiant/${user.id}`}
                            >
