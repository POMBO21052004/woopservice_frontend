...
export default function ViewFormateursByFormateurs() {
  // États
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const res = await api.get("/formateur/view/formateur", { params });
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({});
    try {
      await api.post("/formateur/store/formateur", form);
      fetchUsers();
  ...
  const handleDeleteConfirmed = async () => {
    try {
      ...
      if (userToDelete) {
        ...
        await api.delete(`/formateur/destroy/formateur/${userToDelete}`);
      } else {
        // Suppression en lot
        await api.post(`/formateur/destroy-group/formateur`, { ids: idsToDelete });
      }
  ...
  const handleDownloadPdf = async (e) => {
    ...
    try {
      ...
      const res = await api.get(`/formateur/download/liste/formateur/pdf?${params.toString()}`, {
        responseType: "blob",
      });

      const filename = `rapport_utilisateurs_(formateurs)_${new Date().toISOString().slice(0, 10)}.pdf`;
  ...
  const handleDownloadExcel = async (e) => {
    ...
    try {
      ...
      const res = await api.get(`/formateur/download/liste/formateur/excel?${params.toString()}`, {
        responseType: "blob",
      });

      const filename = `rapport_utilisateurs_(formateurs)_${new Date().toISOString().slice(0, 10)}.xlsx`;
      fileDownload(res.data, filename);
  ...
  return (
    <FormateurLayout>
      <Container className="py-4">
                        ...
                        <td className="text-center">
                          <div className="d-flex gap-1 justify-content-center">
                            <Button
                              ...
                              as={Link}
                              to={`/formateur/show/formateur/${user.id}`}
                            >
                            ...
                            <Button
                              ...
                              title="Modifier"
                              as={Link}
                              to={`/formateur/edit/formateur/${user.id}`}
                            >
                      ...
                      <td colSpan="6" className="text-center py-5">
                        <div className={`${theme === "dark" ? "text-light" : "text-muted"}`}>
                          ...
                          <div>
                            ...
                            <p className="small mb-0">Aucun administrateur ne correspond à vos critères de recherche.</p>
                          </div>
                        </div>
                      </td>
                    </tr>
        ...
        >
          <Modal.Header className={theme === "dark" ? "bg-dark text-light border-secondary" : ""} closeButton>
            <Modal.Title>Inviter un Administrateur</Modal.Title>
          </Modal.Header>
