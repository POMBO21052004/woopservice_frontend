...
export default function ShowEtudiantsZoneAttenteByFormateurs() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/formateur/show/etudiant-espace-attente/${id}`);
  ...
  const handleToggleRole = async () => {
    try {
      await api.patch(`/formateur/toggle-role/etudiant-espace-attente/${id}`);
      const response = await api.get(`/formateur/show/etudiant-espace-attente/${id}`);
      setUser(response.data.user);
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/formateur/destroy/etudiant-espace-attente/${id}`);
      showToastMessage("Étudiant supprimé avec succès", 'success');
      setTimeout(() => {
        navigate('/formateur/view/etudiant-espace-attente');
      }, 1500);
    } catch (err) {
      console.error(err);
            ...
            <Card.Body>
              <div className="d-grid gap-2">
                ...
                <Button 
                  variant="primary"
                  as={Link}
                  to={`/formateur/edit/etudiant-espace-attente/${id}`}
                >
