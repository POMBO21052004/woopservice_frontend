...
export default function FormateurEditByFormateurs() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/formateur/edit/formateur/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post(`/formateur/update/formateur/${id}`, formDataToSend, {
        headers: {
      ...
      if (response.data.status === 'success') {
        ...
        setTimeout(() => {
          navigate(`/formateur/show/formateur/${id}`);
        }, 2000);
      }
    } catch (err) {
  ...
  const handlePasswordUpdate = async (e) => {
    ...
    try {
      ...
      const response = await api.put(`/formateur/update/formateur/${id}`, passwordData);
    ...
    return (
      <FormateurLayout>
        ...
        <div className="d-flex gap-2">
          <Button variant="secondary" as={Link} to="/formateur/view/formateur">
        ...
        <div>
          <div className="d-flex align-items-center mb-2">
            <Button 
              ...
              size="sm"
              as={Link} 
              to={`/formateur/show/formateur/${id}`}
              className="me-3"
            >
              <i data-feather="arrow-left" className="me-1" style={{ width: '16px', height: '16px' }} />
              Retour
            </Button>
            <h1 className="h3 mb-0">Modifier le formateur</h1>
          </div>
                  ...
                  <Col md={6} className="mb-3">
                    <Form.Group>
                      ...
                      <Form.Text className="text-muted">
                        Numéro de contact du formateur
                      </Form.Text>
                    </Form.Group>
                  </Col>
                </Row>
            ...
            <Card className={`border-0 shadow-sm mb-4 ${theme === "dark" ? "bg-dark" : "bg-white"}`}>
              ...
              <Card.Body>
                <div className="small">
                  ...
                  <div className="d-flex align-items-start mb-2">
                    <i data-feather="shield" className="text-primary me-2 mt-1" style={{ width: '14px', height: '14px' }} />
                    <span>Les formateurs ont accès aux fonctionnalités pédagogiques de base</span>
                  </div>
