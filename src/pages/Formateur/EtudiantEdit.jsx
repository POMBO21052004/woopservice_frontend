...
export default function EtudiantEditByFormateurs() {
  const { id } = useParams();
    ...
    const fetchData = async () => {
      try {
        ...
        const [userResponse, classroomResponse] = await Promise.all([
          api.get(`/formateur/edit/etudiant/${id}`),
          api.get('/formateur/view/etudiant') // Pour récupérer les classes
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post(`/formateur/update/etudiant/${id}`, formDataToSend, {
        headers: {
      ...
      if (response.data.status === 'success') {
        ...
        setTimeout(() => {
          navigate(`/formateur/show/etudiant/${id}`);
        }, 2000);
      }
    } catch (err) {
  ...
  const handlePasswordUpdate = async (e) => {
    ...
    try {
      ...
      const response = await api.put(`/formateur/update/etudiant/${id}`, passwordData);
