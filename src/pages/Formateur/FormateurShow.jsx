...
export default function FormateurShowByFormateurs() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/formateur/show/formateur/${id}`);
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/formateur/destroy/formateur/${id}`);
      showToastMessage("Formateur supprimé avec succès", 'success');
      setTimeout(() => {
        navigate('/formateur/view/formateur');
      }, 1500);
    } catch (err) {
      console.error(err);
            ...
            <Card.Body>
              ...
              <div className="small">
                ...
                <div className="d-flex justify-content-between">
                  ...
                  <Badge bg="info">Administrateur (Rôle 2)</Badge>
                </div>
              </div>
            </Card.Body>
          </Card>
