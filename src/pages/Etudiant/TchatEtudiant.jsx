...
import { 
  ...
  Toast, ToastContainer, OverlayTrigger, Tooltip
...
export default function ChatEtudiant() {
  ...
  useEffect(() => {
    feather.replace();
  }, [conversations, messages, searchResults, selectedConversation, currentUser, editingMessage]);
  ...
  const fetchConversations = useCallback(async () => {
    try {
      ...
      const response = await api.get('/etudiant/chat/conversations');
  ...
  const fetchMessages = useCallback(async (conversationMatricule, page = 1, silent = false) => {
    try {
      const response = await api.get(`/etudiant/chat/conversations/${conversationMatricule}/messages`, {
  ...
  const searchUsers = useCallback(async (search) => {
    ...
    try {
      const response = await api.get('/etudiant/chat/users/search', {
  ...
  const startPrivateConversation = async (matriculeDestinataire) => {
    try {
      const response = await api.post('/etudiant/chat/conversations/start', {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (editingMessage) {
        await api.put(`/etudiant/chat/messages/${editingMessage.matricule}/edit`, {
          contenu: messageText.trim()
        });
        setEditingMessage(null);
      } else {
        await api.post('/etudiant/chat/messages/send', formData, {
  ...
  const deleteMessage = async (messageMatricule) => {
    ...
    try {
      await api.delete(`/etudiant/chat/messages/${messageMatricule}`);
      await fetchMessages(selectedConversation.matricule);
  ...
  const togglePinMessage = async (messageMatricule) => {
    try {
      await api.put(`/etudiant/chat/messages/${messageMatricule}/toggle-pin`);
      await fetchMessages(selectedConversation.matricule);
  ...
  const fetchPinnedMessages = async (conversationMatricule) => {
    try {
      const response = await api.get(`/etudiant/chat/conversations/${conversationMatricule}/pinned-messages`);
  ...
  const searchInMessages = async (query) => {
    ...
    try {
      const response = await api.get(`/etudiant/chat/conversations/${selectedConversation.matricule}/search`, {
  ...
  const getConversationDisplayName = (conversation) => {
    if (conversation.autres_participants && conversation.autres_participants.length === 1) {
  ...
  const handleFileSelect = (e) => {
    ...
    if (files.length > 3) {
      showToastMessage('Maximum 3 fichiers autorisés', 'warning');
      return;
    }
    ...
    return (
      <EtudiantLayout>
        <div className="text-center py-5">
          ...
          <p className="mt-2">Chargement du tchat...</p>
        </div>
      </EtudiantLayout>
    );
  }
              ...
              >
                {conversations.length > 0 ? (
                  <ListGroup variant="flush">
                    {conversations
                      ...
                      <ListGroup.Item
                        ...
                        className={`border-1 rounded ${theme === "dark" ? "bg-dark text-light" : ""}`}
                        ...
                        <div className="d-flex align-items-center">
                          <div className="position-relative me-3">
                            {getConversationAvatar(conversation) ? (
                              ...
                              >
                                <i 
                                  data-feather={conversation.autres_participants?.length === 1 ? 'user' : 'users'} 
                          ...
                          <div className="flex-grow-1 min-w-0">
                            ...
                            <div className="d-flex align-items-center gap-2">
                              {conversation.autres_participants?.length > 0 && conversation.autres_participants[0].role === 2 && (
                                <Badge bg="success" className="small">Admin</Badge>
                              )}
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="text-center py-5 px-3">
                    <i data-feather="message-circle" className="text-muted mb-3" style={{ width: "48px", height: "48px" }}></i>
                    <h6 className={theme === "dark" ? "text-light" : "text-muted"}>Aucune conversation</h6>
                    <p className="small text-muted">Commencez une nouvelle conversation</p>
                    <Button 
                      variant="primary" 
                      size="sm"
                      onClick={() => setShowNewChatModal(true)}
                    >
                      <i data-feather="plus" className="me-1" style={{ width: "16px", height: "16px" }}></i>
          ...
          <div className="flex-grow-1 d-flex flex-column">
            {selectedConversation ? (
              <>
                  ...
                  <div className="d-flex justify-content-between align-items-center">
                    <div className="d-flex align-items-center">
                      {getConversationAvatar(selectedConversation) ? (
                        ...
                        >
                          <i 
                            data-feather={selectedConversation.autres_participants?.length === 1 ? 'user' : 'users'} 
                            className="text-white" 
                      ...
                      <div>
                        ...
                        <small className="text-muted">
                          {selectedConversation.autres_participants?.length === 1 
                            ? selectedConversation.autres_participants[0].role === 2 
                              ? 'Admin' 
                              : 'Étudiant'
                ...
                <div className={`border-top p-3 ${theme === "dark" ? "border-secondary" : ""}`}>
                  {/* Message de réponse */}
                  {replyingTo && (
                    <div className={`mb-2 p-2 rounded ${theme === "dark" ? "bg-secondary bg-opacity-25" : "bg-light"}`}>
                      <div className="d-flex justify-content-between align-items-center">
                        <div className="small">
                          <i data-feather="corner-up-left" className="me-1" style={{ width: "12px", height: "12px" }}></i>
                    ...
                    <div className={`mb-2 p-2 rounded ${theme === "dark" ? "bg-warning bg-opacity-25" : "bg-warning bg-opacity-10"}`}>
                      <div className="d-flex justify-content-between align-items-center">
                        <div className="small text-warning">
                          <i data-feather="edit-2" className="me-1" style={{ width: "12px", height: "12px" }}></i>
                  ...
                  <InputGroup>
                    ...
                    >
                      <i data-feather={editingMessage ? "check" : "send"} style={{ width: "16px", height: "16px" }}></i>
                    </Button>
                  </InputGroup>
              ...
              <div className={`h-100 d-flex align-items-center justify-content-center ${theme === "dark" ? "bg-dark" : "bg-light"}`}>
                <div className="text-center">
                  {/* <i data-feather="message-circle" className="text-muted mb-3" style={{ width: "64px", height: "64px" }}></i> */}
                  <FeatherIcon icon="message-circle" style={{ width: "64px", height: "64px" }} />
                  <h5 className={theme === "dark" ? "text-light" : "text-muted"}>Sélectionnez une conversation</h5>
                  <p className="text-muted">Choisissez une conversation dans la liste pour commencer à chatter</p>
                </div>
              </div>
            )}
          </div>
            ...
            {searchResults.length > 0 && (
              <div className="border rounded p-2" style={{ maxHeight: '300px', overflowY: 'auto' }}>
                {searchResults.map((user) => (
                  <div
                    key={user.id}
                    className={`d-flex align-items-center p-2 rounded cursor-pointer ${theme === "dark" ? "hover-bg-secondary" : "hover-bg-light"}`}
                    onClick={() => startPrivateConversation(user.matricule)}
                    style={{ cursor: 'pointer' }}
                  >
                    <Image
        ...
        >
          ...
          <Modal.Body className={theme === "dark" ? "bg-dark text-light" : ""}>
            ...
            <div className="mt-3">
              ...
              >
                {/* <i data-feather="search" className="me-2" style={{ width: "16px", height: "16px" }}></i> */}
                Rechercher
              </Button>
              <Button 
                variant="outline-secondary" 
          ...
          <Modal.Body className={theme === "dark" ? "bg-dark text-light" : ""}>
            {pinnedMessages.length > 0 ? (
              ...
              <div className="text-center py-4">
                <i data-feather="bookmark" className="text-muted mb-3" style={{ width: "48px", height: "48px" }}></i>
