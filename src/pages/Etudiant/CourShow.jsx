...
import { Card, Row, Col, Button, Alert, Spinner, Badge, Toast, ToastContainer, Image } from "react-bootstrap";
...
export default function CoursShowEtudiant() {
  const { id } = useParams();
  ...
  const fetchCours = useCallback(async () => {
    try {
      setLoading(true);
      const response = await api.get(`/etudiant/show/cours/${id}`);
      
      if (response.data.status === 'success') {
        setCours(response.data.cours);
      } else {
        setError(response.data.message || 'Erreur lors du chargement du cours');
      }
    } catch (err) {
      setError('Erreur lors du chargement du cours');
      console.error('Erreur:', err);
      if (err.response?.status === 404) {
        setError('Cours introuvable ou non accessible');
      } else if (err.response?.status === 403) {
        setError('Vous n\'avez pas l\'autorisation d\'accéder à ce cours');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleDownloadFile = async (fileIndex, fichierNom) => {
    try {
      const response = await api.get(`/etudiant/download/cours/${id}/file/${fileIndex}`, {
        responseType: 'blob'
      });
    ...
    return (
      <EtudiantLayout>
        ...
        <div className="d-flex gap-2">
          <Button variant="secondary" as={Link} to="/etudiant/view/cours/my-classroom">
    ...
    <EtudiantLayout>
          ...
          <div className="d-flex align-items-center mb-2">
            ...
            <div>
              ...
              <div className="d-flex align-items-center gap-2 mt-1">
                <small className="text-muted">Code: {cours.matricule}</small>
      ...
      <Row>
          ...
          <Card className={`border-0 shadow-sm mb-4 ${theme === "dark" ? "bg-dark" : "bg-white"}`}>
            ...
            <Card.Body>
              <Row className="g-4">
                ...
                <Col md={6}>
                  <div className="mb-3">
                    <label className="small fw-bold text-muted">CODE DU COURS</label>
                    <div className={`${theme === "dark" ? "text-light" : "text-dark"}`}>
                ...
                <Col md={6}>
                  <div className="mb-3">
                    <label className="small fw-bold text-muted">DATE DE PUBLICATION</label>
                    <div className={`${theme === "dark" ? "text-light" : "text-dark"}`}>
              ...
              <Card.Body>
                <div className="d-flex flex-column gap-3">
                  {cours.liens_externes_decoded.map((lien, index) => (
                    <div key={index} className="p-3 border rounded bg-light">
                      <div className="d-flex align-items-center">
                        ...
                        >
                          <i data-feather="external-link" style={{ width: '14px', height: '14px' }} />
                        </Button>
                      </div>
                    </div>
                  ))}
        ...
        <Col lg={4}>
            ...
            <Card className={`border-0 shadow-sm mb-4 ${theme === "dark" ? "bg-dark" : "bg-white"}`}>
              ...
              <Card.Body>
                {cours.fichiers_joints_1_decoded && (
                  <div className="mb-3">
                    <label className="small fw-bold text-muted mb-2 d-block">DOCUMENT 1</label>
                ...
                {cours.fichiers_joints_2_decoded && (
                  <div>
                    <label className="small fw-bold text-muted mb-2 d-block">DOCUMENT 2</label>
          ...
          {/* Actions rapides pour l'étudiant */}
          <Card className={`border-0 shadow-sm ${theme === "dark" ? "bg-dark" : "bg-white"}`}>
            <Card.Header className="border-0">
              <h5 className="mb-0 d-flex align-items-center">
                <i data-feather="bookmark" className="me-2" style={{ width: '20px', height: '20px' }} />
                Actions
              </h5>
            </Card.Header>
            <Card.Body>
