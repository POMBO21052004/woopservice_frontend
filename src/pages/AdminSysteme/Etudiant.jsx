...
export default function ViewEtudiants() {
  // États
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const res = await api.get("/admin-systeme/view/etudiant", { params });
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({});
    try {
      await api.post("/admin-systeme/store/etudiant", form);
      fetchUsers();
  ...
  const handleDeleteConfirmed = async () => {
    try {
      ...
      if (userToDelete) {
        ...
        await api.delete(`/admin-systeme/destroy/etudiant/${userToDelete}`);
      } else {
        // Suppression en lot
        await api.post(`/admin-systeme/destroy-group/etudiant`, { ids: idsToDelete });
      }
  ...
  const handleDownloadPdf = async (e) => {
    ...
    try {
      ...
      const res = await api.get(`/admin-systeme/download/liste/etudiant/pdf?${params.toString()}`, {
        responseType: "blob",
      });

      const filename = `rapport_utilisateurs_(etudiants)_${new Date().toISOString().slice(0, 10)}.pdf`;
  ...
  const handleDownloadExcel = async (e) => {
    ...
    try {
      ...
      const res = await api.get(`/admin-systeme/download/liste/etudiant/excel?${params.toString()}`, {
        responseType: "blob",
      });

      const filename = `rapport_utilisateurs_(etudiants)_${new Date().toISOString().slice(0, 10)}.xlsx`;
      fileDownload(res.data, filename);
                        ...
                        <td className="text-center">
                          <div className="d-flex gap-1 justify-content-center">
                            <Button
                              ...
                              as={Link}
                              to={`/admin-systeme/show/etudiant/${user.id}`}
                            >
                            ...
                            <Button
                              ...
                              title="Modifier"
                              as={Link}
                              to={`/admin-systeme/edit/etudiant/${user.id}`}
                            >
