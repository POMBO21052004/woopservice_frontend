...
export default function EtudiantShow() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/admin-systeme/show/etudiant/${id}`);
  ...
  const handleToggleRole = async () => {
    try {
      await api.patch(`/admin-systeme/toggle-role/etudiant/${id}`);
      const response = await api.get(`/admin-systeme/show/etudiant/${id}`);
      setUser(response.data.user);
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/admin-systeme/destroy/etudiant/${id}`);
      showToastMessage("Étudiant supprimé avec succès", 'success');
      setTimeout(() => {
        navigate('/admin-systeme/view/etudiants');
      }, 1500);
    } catch (err) {
      console.error(err);
