...
export default function ViewEtudiantsEspaceAttente() {
  // États
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const res = await api.get("/admin-systeme/view/etudiant-espace-attente", { params });
      setUsers(res.data.users || []);
      setClassrooms(res.data.classrooms || []);
    } catch (err) {
      console.error("Erreur lors de la récupération des utilisateurs", err);
    }
  }, [search, createdAtFilter, statusFilter, genderFilter]);

  useEffect(() => {
    feather.replace();
    fetchUsers();
  }, [fetchUsers, setCheckedUsers]);
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({});
    try {
      await api.post("/admin-systeme/store/etudiant-espace-attente", form);
      fetchUsers();
  ...
  const handleInvite = async (e) => {
    ...
    try {
      ...
      setInviteForm({ email: "", matricule_classroom: "", role: "0" });
    } catch (err) {
      if (err.response?.status === 422) {
        setErrors(err.response.data.errors || {});
  ...
  const handleDeleteConfirmed = async () => {
    try {
      ...
      if (userToDelete) {
        ...
        await api.delete(`/admin-systeme/destroy/etudiant-espace-attente/${userToDelete}`);
      } else {
        // Suppression en lot
        await api.post(`/admin-systeme/destroy-group/etudiant-espace-attente`, { ids: idsToDelete });
      }
  ...
  const handleDownloadPdf = async (e) => {
    ...
    try {
      ...
      const res = await api.get(`/admin-systeme/download/liste/etudiant-espace-attente/pdf?${params.toString()}`, {
        responseType: "blob",
      });

      const filename = `rapport_utilisateurs_(etudiants_espace_attente)_${new Date().toISOString().slice(0, 10)}.pdf`;
  ...
  const handleDownloadExcel = async (e) => {
    ...
    try {
      ...
      const res = await api.get(`/admin-systeme/download/liste/etudiant-espace-attente/excel?${params.toString()}`, {
        responseType: "blob",
      });

      const filename = `rapport_utilisateurs_(etudiants_espace_attente)_${new Date().toISOString().slice(0, 10)}.xlsx`;
      fileDownload(res.data, filename);
  ...
  return (
    <AdminSystemeLayout>
      <Container className="py-4">
        ...
        <div className="mb-4">
          <div className="d-flex align-items-center mb-3">
            ...
            <h2 className={`mb-1 ${theme === "dark" ? "text-light" : "text-dark"}`}>
                Gestion des Etudiants En Espace Attente
              </h2>
                        ...
                        <td className="text-center">
                          <div className="d-flex gap-1 justify-content-center">
                            <Button
                              ...
                              as={Link}
                              to={`/admin-systeme/show/etudiant-espace-attente/${user.id}`}
                            >
                            ...
                            <Button
                              ...
                              title="Modifier"
                              as={Link}
                              to={`/admin-systeme/edit/etudiant-espace-attente/${user.id}`}
                            >
                    ...
                    <tr>
                      <td colSpan="7" className="text-center py-5">
                        <div className={`${theme === "dark" ? "text-light" : "text-muted"}`}>
                          ...
                          <div>
                            <h6>Aucun Etudiant trouvé</h6>
                            <p className="small mb-0">Aucun etudiant ne correspond à vos critères de recherche.</p>
                          </div>
                        </div>
                      </td>
                    </tr>
        ...
        >
          <Modal.Header className={theme === "dark" ? "bg-dark text-light border-secondary" : ""} closeButton>
            <Modal.Title>Inviter un Etudiant</Modal.Title>
          </Modal.Header>
