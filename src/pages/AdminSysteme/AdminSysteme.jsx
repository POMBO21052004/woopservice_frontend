...
export default function AdminSysteme() {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const res = await api.get("/admin-systeme/view/admin-systeme", { params });
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({});
    try {
      await api.post("/admin-systeme/store/admin-systeme", form);
      fetchUsers();
  ...
  const handleDeleteConfirmed = async () => {
    try {
      ...
      if (userToDelete) {
        ...
        await api.delete(`/admin-systeme/destroy/admin-systeme/${userToDelete}`);
      } else {
        // Suppression en lot
        await api.post(`/admin-systeme/destroy-group/admin-systeme`, { ids: idsToDelete });
      }
  ...
  const handleDownloadPdf = async (e) => {
    ...
    try {
      ...
      const res = await api.get(`/admin-systeme/download/liste/admin-systeme/pdf?${params.toString()}`, {
        responseType: "blob",
      });

      const filename = `rapport_utilisateurs_(administrateurs_systeme)_${new Date().toISOString().slice(0, 10)}.pdf`;
  ...
  const handleDownloadExcel = async (e) => {
    ...
    try {
      ...
      const res = await api.get(`/admin-systeme/download/liste/admin-systeme/excel?${params.toString()}`, {
        responseType: "blob",
      });

      const filename = `rapport_utilisateurs_(administrateurs_systeme)_${new Date().toISOString().slice(0, 10)}.xlsx`;
      fileDownload(res.data, filename);
                        ...
                        <td className="text-center">
                          <div className="d-flex gap-1 justify-content-center">
                            <Button
                              ...
                              as={Link}
                              to={`/admin-systeme/show/admin-systeme/${user.id}`}
                            >
                            ...
                            <Button
                              ...
                              title="Modifier"
                              as={Link}
                              to={`/admin-systeme/edit/admin-systeme/${user.id}`}
                            >
                      ...
                      <td colSpan="6" className="text-center py-5">
                        <div className={`${theme === "dark" ? "text-light" : "text-muted"}`}>
                          ...
                          <div>
                            ...
                            <p className="small mb-0">Aucun administrateur système ne correspond à vos critères de recherche.</p>
                          </div>
                        </div>
                      </td>
                    </tr>
