...
export default function ShowEtudiantsZoneAttente() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/admin-systeme/show/etudiant-espace-attente/${id}`);
  ...
  const handleToggleRole = async () => {
    try {
      await api.patch(`/admin-systeme/toggle-role/etudiant-espace-attente/${id}`);
      const response = await api.get(`/admin-systeme/show/etudiant-espace-attente/${id}`);
      setUser(response.data.user);
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/admin-systeme/destroy/etudiant-espace-attente/${id}`);
      showToastMessage("Étudiant supprimé avec succès", 'success');
      setTimeout(() => {
        navigate('/admin-systeme/view/etudiant-espace-attente');
      }, 1500);
    } catch (err) {
      console.error(err);
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <div className="d-flex gap-2">
          ...
          <Button 
            variant="primary" 
            as={Link} 
            to={`/admin-systeme/edit/etudiant/${id}`}
          >
            ...
            <Card.Body>
              <div className="d-grid gap-2">
                ...
                <Button 
                  variant="primary"
                  as={Link}
                  to={`/admin-systeme/edit/etudiant-espace-attente/${id}`}
                >
